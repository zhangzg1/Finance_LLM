    ...
    def __init__(self, ptuning_type: PtuningType):
        ...
        if ptuning_type is PtuningType.Classify or ptuning_type is PtuningType.NL2SQL or ptuning_type is PtuningType.Keywords:
            ...
            if ptuning_type is PtuningType.Classify:
                ...
                check_point_path = os.path.join(cfg.CLASSIFY_CHECKPOINT_PATH, "model.safetensors")
            ...
            # 如果是提取关键词类型，加载长度为256的KEYWORDS训练权重
                ...
                check_point_path = os.path.join(cfg.KEYWORDS_CHECKPOINT_PATH, "model.safetensors")
                self.model = AutoModel.from_pretrained(self.model_name, config=self.config, trust_remote_code=True)
            ...
            # 如果是分类类型，加载长度为2048的NL2SQL训练权重
                ...
                check_point_path = os.path.join(cfg.NL2SQL_CHECKPOINT_PATH, "model.safetensors")
                self.model = AutoModel.from_pretrained(self.model_name, config=self.config, trust_remote_code=True)
                self.isNL2SQL = True
            # 修改2：使用safetensors加载权重
            try:
                prefix_state_dict = load_file(check_point_path)
            except Exception as e:
                raise ValueError(f"Error loading safetensors file: {str(e)}")

            new_prefix_state_dict = {}
            for k, v in prefix_state_dict.items():
                if k.startswith("transformer.prefix_encoder."):
                    new_prefix_state_dict[k[len("transformer.prefix_encoder."):]] = v
            self.model.transformer.prefix_encoder.load_state_dict(new_prefix_state_dict, strict=False)
