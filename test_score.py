...
class countScore():
    ...
    def __init__(self):
        ...
        self.simModel_path = 'models/text2vec-base-chinese'  # 相似度模型路径
        ...
        for i in range(len(self.sys_data_type1_list)):
            ...
            if key_word != '无|不|没有|未|否|非|莫|抱歉|毋':
                ...
                if key_value in per_answer:
                    score_type1 += 0.25
                    score_type1 += \
                        semantic_search(self.simModel.encode([per_answer]), self.simModel.encode(sys_answer), top_k=1)[
                            0][0]['score'] * 0.5
    ...
    # 根据用户提供的预测文件路径（Per_path），加载用户回答数据，并对不同类型的问题进行评分
...
if __name__ == "__main__":
    ...
    # 标准答案路径
    standard_path = "./pdf_data/test/answer.json"
    print("Read standard from %s" % standard_path)

    # 预测文件路径
    input_path = "./pdf_data/result_20250311.json"
    print("Read predict file from %s" % input_path)

    out_path = "./pdf_data/test/output.json"
